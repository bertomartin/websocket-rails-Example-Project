...
class Chat.Controller
    ...
    html =
      ...
      <div class="message" style="display:none">
        <label class="label label-info">
          #{message.user_name} #{message.received}
        </label>&nbsp;
        #{message.msg_body}
      </div>
      """
  ...
  bindEvents: =>
    ...
    $('#save-user-info').click @updateUserInfo
  ...
  updateUserInfo: (event) =>
    event.preventDefault()
    @user.user_name = $('#user-name').val()
    @user.full_name = $('#full-name').val()
  ...
  createGuestUser: =>
    @user = new Chat.User('Guest','Guest User')
